...
const SearchBar: React.FC<Props> = ({
  ...
  const results = useMemo(() => {
    ...
    entities.forEach((entity) => {
      ...
      const entityCommentSearch = entity.comment
        ? searchText(entity.comment, query)
        : emptySearch;
      const entityScore =
        (entityNameSearch.matchRatio + entityCommentSearch.matchRatio) / 2;

      if (entityScore > 0) {
        const entitySplitText = entityNameSearch.splitText;
        if (entity.comment)
          entitySplitText.push("  •  ", ...entityCommentSearch.splitText);
      ...
      entity.columns.forEach((column) => {
        ...
        const columnCommentSearch = column.comment
          ...
          : emptySearch;
        const columnScore =
          (columnNameSearch.matchRatio + columnCommentSearch.matchRatio) / 2;

        if (columnScore > 0) {
          const columnSplitText = [
            entity.friendlyName,
            "  •  ",
            ...columnNameSearch.splitText,
          ];
          if (column.comment)
            columnSplitText.push("  •  ", ...columnCommentSearch.splitText);
